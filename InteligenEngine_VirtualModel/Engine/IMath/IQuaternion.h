...
//            else
        ...
        // ...
        IVector3D<T> GetEulerAngleGimbalLock(RotSeq rotSeq = RotSeq::xyz )
        {
...
//        }
        ...
        /// The angle between the vectors is simple to find: the dot product gives its cosine.
        /// The needed axis is also simple to find: it’s the cross product of the two vectors.
        SIMD_INLINE IQuaternion<T> RotationBetweenVectors(const IVector3D<T> _start, const IVector3D<T> _dest)
        {
            ...
            T s = ISqrt( T((1.+cosTheta)*2.) );
            T invs = T(1./s);

            return IQuaternion<T>( s * T(0.5f),  rotationAxis * invs);
